...
const PostCard =({post}) => {
    ...
    console.log(liked)
   /*  const [liked , setLiked] = useState(false);  지운다 */
    const [commentFormOpened, setCommentFormOpened] =useState(false)
    const {removePostLoading,} =useSelector((state) => state.post)
    /* const id = me?.id ==>새로생긴 문법 ( chaining 연산)
    cosnt id = useSelector((state) => state.user.me?.id) */
        ...
        <Card
            ...
            >
                ...
                avatar={(
                     ...
                     <Avatar>{post.User.nickname[0]}</Avatar></a>
                     </Link>
                     )}
